            ...
            case 3:
                ...
                if (reply != null) {
                    ...
                    if (reply.getPerformative() == ACLMessage.INFORM) {
                        // Purchase successful.
                        ((BookBuyerAgent) myAgent).addTransaction(bookType, targetBookTitle.split(":")[1], bestPrice,
                                reply.getSender().getLocalName());
                        // System.out.println(myAgent.getAID().getLocalName() + " purchased " +
                        // _titlesToPurchase.get(_purchasedTitles.size()) +" for "+ bestPrice + " from
                        // "+reply.getSender().getLocalName() );
                        _purchasedTitles.add(_titlesToPurchase.get(_purchasedTitles.size()));
                    } else {
                        System.out.println(
                                myAgent.getAID().getLocalName() + " Attempt failed: requested book already sold.");
                    }
